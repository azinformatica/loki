import azSearchUrlBuilder from '../../src/utils/AzSearchUrlBuilder'
...
describe('SearchUrlBuilder', () => {
    ...
    it('Deve retornar a url construída com os filtros SEM paginação e SEM ordenação', () => {
        pagination = undefined
        expect(azSearchUrlBuilder.build(pathUrl, filters)).toMatch('/pregao?status=ABERTO&responsavel=SAD&edital=0001/2018')
    })

    it('Deve retornar a url construída SEM filtros, com paginação e ordenação CRESCENTE', () => {
        filters = undefined
        expect(azSearchUrlBuilder.build(pathUrl, filters, pagination)).toMatch('/pregao?page=1&size=10&sort=dataAbertura&direction=ASC')
    })

    it('Deve retornar a url construída com os filtros, paginação e ordenação CRESCENTE', () => {
        expect(azSearchUrlBuilder.build(pathUrl, filters, pagination)).toMatch('/pregao?status=ABERTO&responsavel=SAD&edital=0001/2018&page=1&size=10&sort=dataAbertura&direction=ASC')
    })

    it('Deve retornar a url construída com os filtros, paginação e ordenação DECRESCENTE', () => {
        pagination.descending = true
        expect(azSearchUrlBuilder.build(pathUrl, filters, pagination)).toMatch('/pregao?status=ABERTO&responsavel=SAD&edital=0001/2018&page=1&size=10&sort=dataAbertura&direction=DESC')
    })
